...
```{sql connection = db}
#SQL Query to find number of users that are old
...
```{r}
plot1 <- ggplot(plot1df, aes(x = birth_year, y = total_of_users_by_age)) + geom_col()
```
...
durationbyage <- dbGetQuery(db,
                            "SELECT duration, birth_year, user_type, start_station_id, end_station_id
                            FROM trips 
                            LEFT JOIN station_months ON trips.start_station_id = station_months.station_id
                            GROUP BY birth_year;"
                            )
